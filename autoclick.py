...
from core.constants import (
    APP_VERSION, GITHUB_API_URL,
    COLOR_BG, COLOR_CARD, COLOR_INNER, COLOR_ACCENT, COLOR_SUCCESS,
    COLOR_DANGER, COLOR_WARNING, COLOR_MUTED,
    GRADIENT_START, GRADIENT_END, BORDER_COLOR
)
...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler(LOG_FILE, encoding='utf-8', maxBytes=5*1024*1024, backupCount=7),
...
class AutoMationApp(ctk.CTk, HotkeyMixin, PresetMixin, EngineMixin, ActionMixin, LogicMixin, VisionMixin, VariablesMixin, PickerMixin, StealthMixin, TabsMixin, UIMixin):
    ...
    def launch_app(self):
        ...
        self._pending_after_ids.append(
    ...
    def setup_bottom_panels(self, parent):
        ...
        ctk.CTkButton(f_t_btn, text="ล็อคหน้าต่าง", command=self.pick_target_window, width=120, height=32, font=("Inter", 11, "bold")).pack(side="left", padx=5)
        ctk.CTkButton(f_t_btn, text="รีเซ็ต", command=self.reset_target_window, width=100, height=32, fg_color="#334155", font=("Inter", 11)).pack(side="left", padx=5)

        self.cb_follow = ctk.CTkCheckBox(f_target, text="ติดตามหน้าต่างอัตโนมัติ (Follow Window)", variable=self.var_follow_window, font=("Inter", 11))
        ...
        ctk.CTkButton(f_pre_head, text="บันทึกไฟล์", width=80, height=24, font=("Inter", 10), command=self.save_presets_to_file).pack(side="right", padx=5)
    ...
    def setup_execution_panel(self, parent):
        ...
        self.btn_run = ctk.CTkButton(f_run, text="เริ่มทำงาน (Global Start)", command=self.run_automation, fg_color=COLOR_SUCCESS, hover_color="#059669", height=45, font=("Inter", 14, "bold"))
    ...
    def setup_footer(self):
        ...
        ctk.CTkLabel(f_foot, text=f"FrankyAutoMate v{APP_VERSION} Premium", font=("Inter", 10, "bold"), text_color="#475569").pack(side="left", padx=20)
        ...
        self.btn_check_update = ctk.CTkButton(f_foot, text="🔄 เช็คอัปเดต", font=("Inter", 10, "bold"), 

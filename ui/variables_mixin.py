...
class VariablesMixin:
    ...
    def setup_vars_tab(self):
        ...
        # --- Header ---
        ctk.CTkLabel(t, text="ตัวแปรและการนับ (Variables)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 10), anchor="w", padx=22)

        # --- Section 1: Set Variable ---
        ctk.CTkLabel(t, text="สร้าง/กำหนดค่าตัวแปร (Set)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(15, 5), anchor="w", padx=22)
        ...
        ctk.CTkLabel(f_in1, text="=", font=("Tahoma", 14, "bold"), text_color=COLOR_ACCENT).pack(side="left")
        ...
        ctk.CTkButton(f_set, text="เพิ่มคำสั่ง Set", command=self.add_var_set_action,
                       fg_color="#334155", hover_color="#475569", height=32).pack(pady=15, padx=20, fill="x")

        # --- Section 2: Math Operation ---
        ctk.CTkLabel(t, text="การคำนวณ (Math)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(15, 5), anchor="w", padx=22)
        ...
        f_in2.pack(pady=15) # Center packing
        ...
        ctk.CTkButton(f_math, text="เพิ่มคำสั่งคำนวณ", command=self.add_var_math_action,
                       fg_color="#334155", hover_color="#475569", height=32).pack(pady=(0, 15), padx=20, fill="x")

        # --- Section 3: Variable IF ---
        ctk.CTkLabel(t, text="เช็คเงื่อนไขตัวแปร (Check Variable)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(15, 5), anchor="w", padx=22)
        ...
        f_in3.pack(pady=15) # Center packing
        
        ctk.CTkLabel(f_in3, text="ถ้า", font=("Tahoma", 12)).pack(side="left")
        ...
        ctk.CTkButton(f_vif, text="เพิ่มเงื่อนไข", command=self.add_var_if_action,
                       fg_color="#2563eb", hover_color="#1d4ed8", height=35).pack(pady=15, padx=20, fill="x")

    def add_var_set_action(self):
        name = self.entry_var_name.get().strip()
        val = self.entry_var_val.get().strip()
        if not name: return messagebox.showwarning("เตือน", "กรุณาใส่ชื่อตัวแปร")
    ...
    def add_var_if_action(self):
        ...
        if not left or not target or target == "(Refresh)":
            return messagebox.showwarning("เตือน", "กรุณาระบุเงื่อนไขและ Label เป้าหมาย")
            
        self.add_action_item({
            "type": "logic_if",
            "condition": "var_compare",
            "left": left,
            "op": op,
            "right": right,
            "target_label": target,

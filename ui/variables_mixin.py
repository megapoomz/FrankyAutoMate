...
class VariablesMixin:
    ...
    def setup_vars_tab(self):
        ...
        # --- Header ---
        ctk.CTkLabel(t, text="ตัวแปรและการนับ (Variables)", font=("Inter", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(10, 5), anchor="w", padx=22)
        ...
        ctk.CTkButton(f_set, text="เพิ่มคำสั่ง Set", command=self.add_var_set_action,
                       fg_color="#334155", hover_color="#475569", height=30).pack(pady=10, padx=20, fill="x")
        ...
        ctk.CTkButton(f_math, text="เพิ่มคำสั่งคำนวณ", command=self.add_var_math_action,
                       fg_color="#334155", hover_color="#475569", height=30).pack(pady=(0, 10), padx=20, fill="x")

        # --- Section 3: Variable IF ---
        ctk.CTkLabel(t, text="เช็คเงื่อนไขตัวแปร (Check Variable)", font=("Inter", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(8, 3), anchor="w", padx=22)
        ...
        ctk.CTkButton(f_vif, text="เพิ่มเงื่อนไข", command=self.add_var_if_action,
                       fg_color="#2563eb", hover_color="#1d4ed8", height=32).pack(pady=8, padx=20, fill="x")

    def add_var_set_action(self):
        name = self.entry_var_name.get().strip()
        val = self.entry_var_val.get().strip()
        if not name: return messagebox.showwarning("เตือน", "กรุณาใส่ชื่อตัวแปร", parent=self)

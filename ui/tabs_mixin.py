...
from core.constants import (
    COLOR_ACCENT, COLOR_BG, COLOR_INNER, COLOR_MUTED, COLOR_DANGER, COLOR_SUCCESS,
    BORDER_COLOR, GRADIENT_START, GRADIENT_END, COLOR_CARD
)
...
class TabsMixin:
    ...
    def setup_click_tab(self):
        ...
        self.btn_pick_coord = ctk.CTkButton(f_top, text="เลือกพิกัด (Pick)", command=self.start_pick_location, 
                                            fg_color=GRADIENT_END, hover_color=GRADIENT_START, width=120, height=32, font=("Tahoma", 11, "bold"))
        ...
        rb_bg = ctk.CTkRadioButton(f_r1, text="เบื้องหลัง (BG)", variable=self.var_click_mode, value="background", font=("Tahoma", 12))
        ...
        cb_stop = ctk.CTkCheckBox(f_act, text="จบงานทันที (Stop After)", variable=self.var_click_stop, 
        ...
        ctk.CTkButton(f_act, text="เพิ่ม (Add)", command=self.add_click_action, 
                       fg_color=COLOR_SUCCESS, hover_color="#059669", height=32, width=100, font=("Tahoma", 12, "bold")).pack(side="right")

    def setup_type_tab(self):
        t = self.tab_type
        ctk.CTkLabel(t, text="พิมพ์ข้อความ / กดปุ่ม (Keyboard Input)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="center")
        
        # Row 1: Entry + Record Button
        f_row1 = ctk.CTkFrame(t, fg_color="transparent")
        f_row1.pack(fill="x", padx=15, pady=(15, 5))
        
        self.entry_text = ctk.CTkEntry(f_row1, placeholder_text="พิมพ์ข้อความ / กดปุ่ม...", height=35, font=("Tahoma", 12))
        self.entry_text.pack(side="left", fill="x", expand=True, padx=(0, 5))
        
        self.btn_record_key = ctk.CTkButton(f_row1, text="อัดปุ่ม (Rec)", command=self.start_recording_action_hotkey, 
                                            fg_color=COLOR_CARD, hover_color=COLOR_INNER, width=80, height=35, font=("Tahoma", 11))
    ...
    def setup_image_tab(self):
        t = self.tab_image
        ctk.CTkLabel(t, text="1. เลือกรูปภาพ (Target Image)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="center")
        ...
        f_img_row.pack(fill="x", padx=15, pady=15)
        
        self.lbl_img_path = ctk.CTkLabel(f_img_row, text="ยังไม่ได้เลือกรูปภาพ...", text_color=COLOR_MUTED, font=("Tahoma", 11, "italic"), anchor="w")
        self.lbl_img_path.pack(side="left", fill="x", expand=True)
        ctk.CTkButton(f_img_row, text="เลือกไฟล์", command=self.browse_image, width=100, height=35, fg_color="#3b82f6", font=("Tahoma", 11, "bold")).pack(side="right")
        ...
        self.lbl_region_info = ctk.CTkLabel(f_reg, text="ทั้งหน้าจอ (Full Screen)", text_color="#94a3b8", font=("Tahoma", 10))
    ...
    def setup_color_tab(self):
        t = self.tab_color
        ctk.CTkLabel(t, text="ตรวจจับสี & คลิก (Color)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="center")
        ...
        ctk.CTkLabel(f_col, text="ข้อมูลสี (RGB)", font=("Tahoma", 11, "bold"), text_color=COLOR_ACCENT).pack(pady=(10, 5))
        self.lbl_color_info = ctk.CTkLabel(f_col, text="ยังไม่ได้ดูดสี", text_color="#94a3b8", font=("Tahoma", 10))
        self.lbl_color_info.pack(pady=2)
        
        self.btn_pick_color = ctk.CTkButton(f_col, text="ดูดสี (Pick)", command=self.start_pick_color, 
                                            fg_color=GRADIENT_END, hover_color=GRADIENT_START, height=32, font=("Tahoma", 11, "bold"))
        self.btn_pick_color.pack(pady=(5, 10), padx=15, fill="x")
        ...
        ctk.CTkLabel(f_tol, text="ความเพี้ยนสี (Tolerance)", font=("Tahoma", 11, "bold"), text_color=COLOR_ACCENT).pack(pady=(10, 5))
        self.entry_tol = ctk.CTkEntry(f_tol, width=70, height=32, justify="center", font=("JetBrains Mono", 13), fg_color=COLOR_BG, border_color=BORDER_COLOR)
        ...
        ctk.CTkLabel(t, text="ADVANCED LOGIC (ตรวจสอบหลายจุด)", font=("Tahoma", 10, "bold"), text_color=COLOR_MUTED).pack(pady=(15, 5), anchor="center")
        ...
        self.lbl_multi_color_count = ctk.CTkLabel(f_multi, text="จุดที่เก็บไว้: 0 จุด", font=("Tahoma", 11), text_color=COLOR_ACCENT)
        self.lbl_multi_color_count.pack(pady=(12, 5))
        ...
        btn_add_pt = ctk.CTkButton(f_mb, text="เก็บจุดสี (Add Point)", command=self.add_multi_color_point, fg_color=COLOR_CARD, hover_color=COLOR_INNER, font=("Tahoma", 11, "bold"), height=32)
        btn_add_pt.grid(row=0, column=0, padx=5, sticky="ew")
        ToolTip(btn_add_pt, "ไปจิ้มสีเพิ่มอีกจุด เพื่อเอามาเช็คพร้อมกัน")
        
        btn_clr_pt = ctk.CTkButton(f_mb, text="ล้างทั้งหมด", command=self.clear_multi_color_points, fg_color=COLOR_DANGER, hover_color=COLOR_DANGER, font=("Tahoma", 11, "bold"), height=32, text_color="white")
        btn_clr_pt.grid(row=0, column=1, padx=5, sticky="ew")
        
        f_logic = ctk.CTkFrame(f_multi, fg_color="transparent")
        f_logic.pack(pady=(5, 15)) # Center pack without fill=x
        
        ctk.CTkLabel(f_logic, text="เงื่อนไข:", font=("Tahoma", 11), text_color="#94a3b8").pack(side="left")
        self.var_multi_color_logic = ctk.StringVar(value="AND")
        
        rb_and = ctk.CTkRadioButton(f_logic, text="ทุกจุด (AND)", variable=self.var_multi_color_logic, value="AND", font=("Tahoma", 12), fg_color=COLOR_ACCENT)
        rb_and.pack(side="left", padx=10)
        ToolTip(rb_and, "ต้องเจอสีครบทุกจุด ถึงจะทำงาน")
        
        rb_or = ctk.CTkRadioButton(f_logic, text="บางจุด (OR)", variable=self.var_multi_color_logic, value="OR", font=("Tahoma", 12), fg_color=COLOR_ACCENT)
        ...
        ctk.CTkCheckBox(f_mc_act, text="จบงานทันที (Stop After)", variable=self.var_multi_color_stop,
                        font=("Tahoma", 11), text_color=COLOR_MUTED, border_color=BORDER_COLOR, width=20, height=20).pack(side="left", padx=5)

        ctk.CTkButton(t, text="เพิ่มการเช็คหลายสี (Multi-Color Action)", command=self.add_multi_color_action, fg_color="#9333ea", hover_color="#7e22ce", height=45, font=("Tahoma", 13, "bold")).pack(pady=(10, 25), padx=20, fill="x")

    def setup_log_tab(self):
        t = self.tab_log
        ctk.CTkLabel(t, text="SYSTEM ACTIVITY", font=("Tahoma", 10, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="center")
        self.txt_log = ctk.CTkTextbox(t, height=350, font=("JetBrains Mono", 11), fg_color=COLOR_BG, border_width=1, border_color=BORDER_COLOR, text_color=COLOR_MUTED)
        ...
        ctk.CTkButton(f_btn, text="ล้าง (Clear)", command=self.clear_logs, height=35, fg_color="#334155", font=("Tahoma", 11, "bold"), width=100).pack(side="left", padx=(0, 10))
        ctk.CTkButton(f_btn, text="Export Log", command=self.export_logs, height=35, fg_color=COLOR_INNER, border_width=1, border_color=BORDER_COLOR, font=("Tahoma", 11, "bold"), width=120).pack(side="left")
        ...
        ctk.CTkCheckBox(f_deb, text="Overlay", variable=self.var_debug_overlay, font=("Tahoma", 11), text_color=COLOR_MUTED, border_color="#334155").pack(side="right", padx=(5, 0))
        ctk.CTkCheckBox(f_deb, text="Debug", variable=self.var_debug_mode, font=("Tahoma", 11), text_color=COLOR_MUTED, border_color="#334155").pack(side="right", padx=(10, 5))
    ...
    def setup_stealth_tab(self):
        t = self.tab_stealth
        ctk.CTkLabel(t, text="โหมดเนียน & ป้องกันการจับได้ (Anti-Detection)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="w", padx=22)
        ...
        cb_move = ctk.CTkCheckBox(f_move, text="ขยับเมาส์โค้งมนแบบคน (Human Curves)", variable=self.var_stealth_move, font=("Tahoma", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets)
        ...
        cb_jitter = ctk.CTkCheckBox(f_jitter, text="สุ่มจุดคลิกรอบเป้าหมาย (Random Jitter)", variable=self.var_stealth_jitter, font=("Tahoma", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets)
        ...
        ctk.CTkLabel(f_j_val, text="รัศมีความมั่ว (Radius):", font=("Tahoma", 11), text_color="#94a3b8").pack(side="left", padx=(30, 10))
        
        self.slider_jitter = ctk.CTkSlider(f_j_val, from_=1, to=20, number_of_steps=19, width=150, variable=self.var_stealth_jitter_radius, progress_color=COLOR_ACCENT, button_color=COLOR_ACCENT, command=lambda v: self.auto_save_presets())
        self.slider_jitter.pack(side="left")
        ToolTip(self.slider_jitter, "ยิ่งเยอะ ยิ่งคลิกไกลจากจุดเดิมมาก")
        
        ctk.CTkLabel(f_j_val, textvariable=self.var_stealth_jitter_radius, font=("JetBrains Mono", 11, "bold"), text_color=COLOR_ACCENT).pack(side="left", padx=10)
        ctk.CTkLabel(f_j_val, text="px", font=("Tahoma", 10), text_color="#64748b").pack(side="left")
        ctk.CTkLabel(t, text="ADVANCED SYSTEM STEALTH", font=("Tahoma", 10, "bold"), text_color=COLOR_MUTED).pack(pady=(15, 5), anchor="w", padx=22)
        ...
        ctk.CTkCheckBox(f_options, text="ซ่อนหน้าต่าง (Auto Minimize)", variable=self.var_stealth_hide_window, font=("Tahoma", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets).pack(side="left", padx=(0, 20))
        ctk.CTkCheckBox(f_options, text="สุ่มชื่อหน้าต่าง (Random Title)", variable=self.var_stealth_random_title, command=self.on_random_title_toggle, font=("Tahoma", 12), fg_color=COLOR_ACCENT, border_color="#334155").pack(side="left")
        f_send = ctk.CTkFrame(f_tech, fg_color="transparent")
        f_send.pack(fill="x", padx=20, pady=(0, 20))
        ctk.CTkCheckBox(f_send, text="ใช้ SendInput API (Low-level Emulation)", variable=self.var_stealth_sendinput, font=("Tahoma", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets).pack(side="left")
        
        # Overlay option
        f_overlay = ctk.CTkFrame(f_tech, fg_color="transparent")
        f_overlay.pack(fill="x", padx=20, pady=(0, 15))
        cb_overlay = ctk.CTkCheckBox(f_overlay, text="แสดงหน้าจอเตือนเมื่อ Auto ทำงาน (Fullscreen Overlay)", variable=self.var_show_overlay, font=("Tahoma", 12), fg_color="#ef4444", border_color="#334155", command=self.auto_save_presets)
        cb_overlay.pack(side="left")
        ToolTip(cb_overlay, "แสดงหน้าจอทึบเต็มจอเพื่อเตือนไม่ให้สัมผัสเมาส์/คีย์บอร์ด\\nระหว่าง Auto ทำงาน (Click-through: ไม่บล็อกการทำงานของ Auto)")
        
        ctk.CTkLabel(t, text="โหมด Stealth อาจทำให้การทำงานช้าลงเพื่อความเสมือนมนุษย์", text_color="#f59e0b", font=("Tahoma", 11, "italic")).pack(pady=10)

    def setup_wait_tab(self):
        t = self.tab_wait
        ctk.CTkLabel(t, text="TIME DELAY CONFIGURATION", font=("Tahoma", 10, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="w", padx=22)
        ...
        ctk.CTkLabel(f_wait, text="หยุดรอเป็นเวลา (วินาที):", font=("Tahoma", 12), text_color="#94a3b8").pack(pady=(20, 5))
        self.entry_wait = ctk.CTkEntry(f_wait, width=200, height=60, font=("JetBrains Mono", 28, "bold"), justify="center", placeholder_text="0.5", fg_color="#020617", border_color=COLOR_ACCENT)
        ...
        ctk.CTkCheckBox(f_wait_act, text="จบงานทันที (Stop After)", variable=self.var_wait_stop,
                        font=("Tahoma", 11), text_color=COLOR_MUTED, border_color=BORDER_COLOR, width=20, height=20).pack(side="left", padx=5)
        ctk.CTkButton(t, text="เพิ่มขั้นตอนการรอ (ADD WAIT ACTION)", command=self.add_wait_action, fg_color=COLOR_SUCCESS, hover_color="#059669", height=50, font=("Tahoma", 14, "bold")).pack(pady=15, padx=20, fill="x")

    def update_list_display(self):
        for widget in self.scroll_actions.winfo_children(): widget.destroy()
        self.action_widgets = []
        self._action_ui_refs = []
        if not self.actions:
        ...
        indents = [0] * len(self.actions)
        label_map = {}
        for i, act in enumerate(self.actions):
            if act["type"] == "logic_label": label_map[act.get("name")] = i
                ...
                if target in label_map:
                    ...
                    if t_idx > i: # Forward Jump
                        for k in range(i + 1, t_idx):
                            ...
                            if act["type"] == "logic_else" and k == i + 1 and self.actions[k]["type"] == "logic_label":
                                continue
                            indents[k] += 1
    ...
    def create_action_widget(self, index, action_data, indent=0):
        ...
        base_padx = 12
        indent_px = indent * 25
        if indent_px > 200: indent_px = 200
        
        f = ctk.CTkFrame(self.scroll_actions, fg_color=base_col, 
        ...
        lbl_idx = ctk.CTkLabel(f_idx, text=f"{index+1}", font=("Tahoma", 9, "bold"), text_color="white")
    ...
    # lbl_icon = ctk.CTkLabel(f_left, text="", font=("Segoe UI Emoji", 16))
    # lbl_icon.pack(pady=(2,0))
    ...
    # --- Middle: Content ---
        ...
        elif t == "text": desc = f"\"{action_data['content']}\""
        elif t == "hotkey": desc = f"Key: {action_data['content'].upper()}"
        elif t == "wait": desc = f"{action_data['seconds']}s"
        elif t == "color_search":
            desc = f"RGB {action_data['rgb']}"
            if action_data.get("region"): desc += " [Region]"
        elif t == "image_search": 
            import os
        ...
        elif t == "multi_color_check": desc = f"{len(action_data.get('points', []))} Points ({action_data.get('logic', 'AND')})"
        elif t == "logic_label": desc = f"ชื่อจุด: {action_data.get('name')}"
        ...
        # Detail Label
        lbl_desc = ctk.CTkLabel(f_content, text=desc, font=("Tahoma", 11), text_color=detail_col, anchor="w")
        lbl_desc.pack(side="left", padx=10)
        lbl_desc.bind("<Button-1>", on_click)

        # --- Right: Badges ---
        badges = []
        if action_data.get("stop_after"): badges.append(("STOP", COLOR_DANGER))
        if t == "logic_label": badges.append(("⚓", None))
        ...
        if badges:
            ...
            for txt, col in badges:
                if col: ctk.CTkLabel(f_right, text=txt, font=("Tahoma", 9, "bold"), text_color=col).pack(side="right")
                else: ctk.CTkLabel(f_right, text=txt, font=("Segoe UI Emoji", 14)).pack(side="right")
                
        self._action_ui_refs.append({'card': f, 'pill': f_idx, 'type': t})

    def select_action(self, index):
        old_idx = getattr(self, 'selected_index', -1)
        if old_idx == index: return
        self.selected_index = index
        
        if not hasattr(self, '_action_ui_refs'): return
        
        # Restore old
        if 0 <= old_idx < len(self._action_ui_refs):
            ref = self._action_ui_refs[old_idx]
            is_label = ref['type'] == "logic_label"
            ref['card'].configure(
                fg_color=("#dbeafe", "#172554") if is_label else COLOR_CARD, 
                border_color=("#3b82f6", "#1d4ed8") if is_label else BORDER_COLOR,
                border_width=1
            )
            ref['pill'].configure(fg_color="#334155")
            
        # Highlight new
        if 0 <= index < len(self._action_ui_refs):
            ref = self._action_ui_refs[index]
            ref['card'].configure(fg_color=COLOR_INNER, border_color=COLOR_ACCENT, border_width=2)
            ref['pill'].configure(fg_color=COLOR_ACCENT)

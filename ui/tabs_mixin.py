...
class TabsMixin:
    ...
    def setup_click_tab(self):
        ...
        self.btn_pick_coord = ctk.CTkButton(f_top, text="เลือกพิกัด (Pick)", command=self.start_pick_location, 
                                            fg_color=GRADIENT_END, hover_color=GRADIENT_START, width=120, height=32, font=("Inter", 11, "bold"))
        ...
        cb_stop = ctk.CTkCheckBox(f_act, text="จบงานทันที (Stop After)", variable=self.var_click_stop, 
                        font=("Inter", 11), text_color=COLOR_MUTED, border_color=BORDER_COLOR, checkmark_color="white", width=20, height=20)
        cb_stop.pack(side="left")

        ctk.CTkButton(f_act, text="เพิ่ม (Add)", command=self.add_click_action, 
                       fg_color=COLOR_SUCCESS, hover_color="#059669", height=32, width=100, font=("Inter", 12, "bold")).pack(side="right")

    def setup_type_tab(self):
        t = self.tab_type
        ctk.CTkLabel(t, text="พิมพ์ข้อความ / กดปุ่ม (Keyboard Input)", font=("Inter", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="center")
        ...
        self.btn_record_key = ctk.CTkButton(f_row1, text="อัดปุ่ม (Rec)", command=self.start_recording_action_hotkey, 
                                            fg_color=COLOR_CARD, hover_color=COLOR_INNER, width=80, height=35, font=("Inter", 11))
        ...
        ctk.CTkCheckBox(f_opts, text="จบงานทันที", variable=self.var_type_stop,
                        font=("Inter", 11), text_color=COLOR_MUTED, border_color=BORDER_COLOR, width=20, height=20).pack(side="left", padx=(10, 0))

        # Right: Add Button
        ctk.CTkButton(f_opts, text="เพิ่ม (Add)", command=self.add_type_action, 
                      fg_color=COLOR_SUCCESS, hover_color="#059669", height=32, width=80, font=("Inter", 12, "bold")).pack(side="right")
    ...
    def setup_image_tab(self):
        ...
        ctk.CTkButton(f_img_row, text="เลือกไฟล์", command=self.browse_image, width=100, height=35, fg_color="#3b82f6", font=("Inter", 11, "bold")).pack(side="right")
        ...
        self.btn_pick_region = ctk.CTkButton(f_reg, text="ตีกรอบหน้าจอ", command=self.start_pick_region, fg_color=GRADIENT_END, hover_color=GRADIENT_START, height=32, font=("Inter", 11, "bold"))
        ...
        self.entry_off_x = ctk.CTkEntry(f_off_row, width=50, height=32, placeholder_text="X", font=("JetBrains Mono", 11), fg_color=COLOR_BG, border_color=BORDER_COLOR, justify="center")
        self.entry_off_x.insert(0, "0")
        self.entry_off_x.pack(side="left", padx=5)
        self.entry_off_y = ctk.CTkEntry(f_off_row, width=50, height=32, placeholder_text="Y", font=("JetBrains Mono", 11), fg_color=COLOR_BG, border_color=BORDER_COLOR, justify="center")
        ...
        menu_mode = ctk.CTkOptionMenu(f_img_btns, variable=self.var_img_click_mode, values=["normal", "background"], width=110, fg_color=COLOR_CARD, button_color=COLOR_INNER)
        ...
        ctk.CTkButton(t, text="เพิ่มคำสั่ง (Add)", command=self.add_image_action, 
                       fg_color=COLOR_SUCCESS, hover_color="#059669", height=40, font=("Inter", 13, "bold")).pack(pady=5, padx=20, fill="x")
    ...
    def setup_color_tab(self):
        ...
        self.btn_pick_color = ctk.CTkButton(f_col, text="ดูดสี (Pick)", command=self.start_pick_color, 
                                            fg_color=GRADIENT_END, hover_color=GRADIENT_START, height=32, font=("Inter", 11, "bold"))
        ...
        ctk.CTkCheckBox(f_color_act, text="จบงานทันที (Stop After)", variable=self.var_color_stop,
                        font=("Inter", 11), text_color=COLOR_MUTED, border_color=BORDER_COLOR, width=20, height=20).pack(side="left", padx=5)
        
        ctk.CTkRadioButton(f_color_act, text="ปกติ", variable=self.var_color_click_mode, value="normal", font=("Inter", 11)).pack(side="right", padx=5)
        rb_bg_color = ctk.CTkRadioButton(f_color_act, text="เบื้องหลัง (BG)", variable=self.var_color_click_mode, value="background", font=("Inter", 11))
        ...
        ctk.CTkButton(t, text="เพิ่มคำสั่งเช็คสี (Add Action)", command=self.add_color_action, 
                       fg_color=COLOR_SUCCESS, hover_color="#059669", height=40, font=("Inter", 13, "bold")).pack(pady=(5, 5), padx=20, fill="x")
        ...
        btn_add_pt = ctk.CTkButton(f_mb, text="เก็บจุดสี (Add Point)", command=self.add_multi_color_point, fg_color=COLOR_CARD, hover_color=COLOR_INNER, font=("Inter", 11, "bold"), height=32)
        btn_add_pt.grid(row=0, column=0, padx=5, sticky="ew")
        ToolTip(btn_add_pt, "ไปจิ้มสีเพิ่มอีกจุด เพื่อเอามาเช็คพร้อมกัน")
        
        btn_clr_pt = ctk.CTkButton(f_mb, text="ล้างทั้งหมด", command=self.clear_multi_color_points, fg_color=COLOR_DANGER, hover_color=COLOR_DANGER, font=("Inter", 11, "bold"), height=32, text_color="white")
        ...
        rb_and = ctk.CTkRadioButton(f_logic, text="ทุกจุด (AND)", variable=self.var_multi_color_logic, value="AND", font=("Inter", 12), fg_color=COLOR_ACCENT)
        rb_and.pack(side="left", padx=10)
        ToolTip(rb_and, "ต้องเจอสีครบทุกจุด ถึงจะทำงาน")
        
        rb_or = ctk.CTkRadioButton(f_logic, text="บางจุด (OR)", variable=self.var_multi_color_logic, value="OR", font=("Inter", 12), fg_color=COLOR_ACCENT)
        ...
        ctk.CTkButton(t, text="เพิ่มการเช็คหลายสี (Multi-Color Action)", command=self.add_multi_color_action, fg_color="#9333ea", hover_color="#7e22ce", height=40, font=("Inter", 13, "bold")).pack(pady=(5, 10), padx=20, fill="x")
    ...
    def setup_log_tab(self):
        ...
        ctk.CTkButton(f_btn, text="ล้าง (Clear)", command=self.clear_logs, height=35, fg_color="#334155", font=("Inter", 11, "bold"), width=100).pack(side="left", padx=(0, 10))
        ctk.CTkButton(f_btn, text="Export Log", command=self.export_logs, height=35, fg_color=COLOR_INNER, border_width=1, border_color=BORDER_COLOR, font=("Inter", 11, "bold"), width=120).pack(side="left")
        ...
        ctk.CTkCheckBox(f_deb, text="Overlay", variable=self.var_debug_overlay, font=("Inter", 11), text_color=COLOR_MUTED, border_color="#334155").pack(side="right", padx=(5, 0))
        ctk.CTkCheckBox(f_deb, text="Debug", variable=self.var_debug_mode, font=("Inter", 11), text_color=COLOR_MUTED, border_color="#334155").pack(side="right", padx=(10, 5))
    ...
    def setup_stealth_tab(self):
        t = self.tab_stealth
        ctk.CTkLabel(t, text="โหมดเนียน & ป้องกันการจับได้ (Anti-Detection)", font=("Inter", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="w", padx=22)
        ...
        cb_move = ctk.CTkCheckBox(f_move, text="ขยับเมาส์โค้งมนแบบคน (Human Curves)", variable=self.var_stealth_move, font=("Inter", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets)
        ...
        cb_jitter = ctk.CTkCheckBox(f_jitter, text="สุ่มจุดคลิกรอบเป้าหมาย (Random Jitter)", variable=self.var_stealth_jitter, font=("Inter", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets)
        ...
        ctk.CTkCheckBox(f_options, text="ซ่อนหน้าต่าง (Auto Minimize)", variable=self.var_stealth_hide_window, font=("Inter", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets).pack(side="left", padx=(0, 20))
        ctk.CTkCheckBox(f_options, text="สุ่มชื่อหน้าต่าง (Random Title)", variable=self.var_stealth_random_title, command=self.on_random_title_toggle, font=("Inter", 12), fg_color=COLOR_ACCENT, border_color="#334155").pack(side="left")
        f_send = ctk.CTkFrame(f_tech, fg_color="transparent")
        f_send.pack(fill="x", padx=20, pady=(0, 20))
        ctk.CTkCheckBox(f_send, text="ใช้ SendInput API (Low-level Emulation)", variable=self.var_stealth_sendinput, font=("Inter", 12), fg_color=COLOR_ACCENT, border_color="#334155", command=self.auto_save_presets).pack(side="left")
        
        ctk.CTkLabel(t, text="โหมด Stealth อาจทำให้การทำงานช้าลงเพื่อความเสมือนมนุษย์", text_color="#f59e0b", font=("Inter", 11, "italic")).pack(pady=10)
    ...
    def setup_wait_tab(self):
        ...
        ctk.CTkCheckBox(f_wait_act, text="จบงานทันที (Stop After)", variable=self.var_wait_stop,
                        font=("Inter", 11), text_color=COLOR_MUTED, border_color=BORDER_COLOR, width=20, height=20).pack(side="left", padx=5)
        ctk.CTkButton(t, text="เพิ่มขั้นตอนการรอ (ADD WAIT ACTION)", command=self.add_wait_action, fg_color=COLOR_SUCCESS, hover_color="#059669", height=50, font=("Inter", 14, "bold")).pack(pady=15, padx=20, fill="x")
    ...
    # --- Middle: Content ---
        ...
        if t == "click":
            ...
            cond_map = {"image_found": "เจอรูปภาพ", "color_match": "สีตรงกัน", "not_image_found": "ไม่เจอรูป", "not_color_match": "สีไม่ตรง", "var_compare": "ตัวแปรตรงเงื่อนไข"}
        ...
        if badges:
            ...
            for txt, col in badges:
                if col: ctk.CTkLabel(f_right, text=txt, font=("Inter", 9, "bold"), text_color=col).pack(side="right")

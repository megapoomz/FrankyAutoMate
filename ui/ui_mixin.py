...
from core.constants import (
    COLOR_SUCCESS, COLOR_ACCENT, COLOR_INNER, COLOR_CARD, BORDER_COLOR, COLOR_MUTED
)
...
class UIMixin:
    ...
    def highlight_action(self, index):
        def _highlight():
            ...
            if 0 <= prev < len(widgets):
                ...
                widgets[prev].configure(
                    fg_color=COLOR_INNER if is_sel else COLOR_CARD,
                    border_color=COLOR_ACCENT if is_sel else BORDER_COLOR
                )
    ...
    def show_click_marker(self, x, y):
        ...
        def _show():
            try:
                ...
                # Move offscreen instead of withdraw() to prevent GDI leak
                self._click_marker_timer = self.after(400, lambda: m.geometry("30x30+-1000+-1000") if m.winfo_exists() else None)
            except: pass
        self.after(0, _show)
    ...
    def show_search_region(self, x, y, w, h):
        """Draws a blue rectangle to show search area (Debug Overlay)"""
        var = getattr(self, 'var_debug_overlay', None)
        if not var or not var.get(): return
        ...
        def _show():
            try:
                ...
                c = ctk.CTkCanvas(m, width=w, height=h, bg="black", highlightthickness=0)
                c.pack()
                c.create_rectangle(0, 0, w, h, outline="#3498db", width=2, dash=(5, 2))
                self.after(150, m.destroy)
            except: pass
        self.after(0, _show)
    ...
    def showtip(self, event=None):
        ...
        try:
            ...
            if platform.system() == "Windows":
                self.tw.wm_attributes("-transparentcolor", "gray") 
        except: pass

        label = ctk.CTkLabel(self.tw, text=self.text, justify='left',
                       bg_color=COLOR_INNER, fg_color=COLOR_INNER, text_color=COLOR_MUTED,
                       corner_radius=6, width=200, wraplength=self.wraplength, font=("Tahoma", 12))

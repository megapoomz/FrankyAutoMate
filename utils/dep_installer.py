...
class DependencyInstaller:
    ...
    def install_pytesseract(callback=None):
        ...
        def run():
            try:
                if callback:
                    callback("กำลังติดตั้งไลบรารี pytesseract...")
                subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", "pytesseract", "requests"])
                if callback:
                    callback("ติดตั้งไลบรารีสำเร็จ! ✅")
            except Exception as e:
                if callback:
                    callback(f"ล้มเหลวในการลงไลบรารี: {e}")
        ...
        def run():
            ...
            for url in TESSERACT_MIRRORS:
                try:
                    ...
                    if callback:
                        callback(f"กำลังลองจาก Mirror: {site_name}...")
                    ...
                    if callback:
                        callback(f"Mirror {site_name} พยายามแล้วแต่ล้มเหลว...")
                    continue
            ...
            try:
                if callback:
                    callback("ดาวน์โหลดเสร็จสิ้น กำลังติดตั้ง (ใช้เวลาสักครู่)...")
                ...
                try:
                    ...
                    if not verify_file_checksum(installer_path, filename=installer_filename):
                        if callback:
                            callback("⚠️ Checksum ไม่ตรง! อาจมีการดัดแปลงไฟล์")
                ...
                if found:
                    if callback:
                        callback("Tesseract-OCR ติดตั้งสำเร็จ! ✅")
                    messagebox.showinfo(
                        "สำเร็จ", "ติดตั้ง Tesseract-OCR เรียบร้อยแล้ว!\nกรุณารีสตาร์ทโปรแกรมอีกครั้งเพื่อเริ่มใช้งาน AI Vision", parent=app
                    )
                else:
                    if callback:
                        callback("ติดตั้งเสร็จสิ้น แต่หาไม่เจอ กรุณาเช็ค Manual")

            except Exception as e:
                if callback:
                    callback(f"เกิดข้อผิดพลาด: {e}")

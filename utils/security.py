...
def dpapi_encrypt(data: bytes) -> bytes:
    ...
    try:
        ...
        input_blob.pbData = ctypes.cast(ctypes.create_string_buffer(data, len(data)),
...
def dpapi_decrypt(encrypted: bytes) -> bytes:
    ...
    try:
        ...
        input_blob.pbData = ctypes.cast(ctypes.create_string_buffer(encrypted, len(encrypted)),
                                         ctypes.POINTER(ctypes.c_char))
...
def save_config_secure(config_data: dict, filepath: str):
    ...
    if _dpapi_available():
        ...
        if encrypted:
            ...
            secure_data = {
                "_encrypted": True,
                "_data": base64.b64encode(encrypted).decode('ascii')
            }

...
class EngineMixin:
    ...
    def execute_one(self, action: Dict[str, Any], current_index: int, run_actions: list = None) -> Optional[int]:
        ...
        try:
            if t == "click":
                ...
                self._execute_multi_color_check(action)
            elif t == "ocr_search":
                self._execute_ocr_search(action)

            # Variable & Logic Engine (NEW Phase 3)
            elif t == "var_set":
                self._execute_var_set(action)
            elif t == "var_math":
                self._execute_var_math(action)
            elif t == "logic_if":
                jump_target = self._execute_logic_if(action, current_index, run_actions)
            elif t == "logic_jump" or t == "logic_else":
                jump_target = self._execute_logic_jump(action, current_index, run_actions)
            elif t == "logic_label":
                pass  # Labels do nothing when hit sequentially
    ...
    def _execute_image_search(self, action: Dict[str, Any]) -> None:
        ...
        while self.is_running:
            try:
                ...
                try:
                    found_loc = pyautogui.locateCenterOnScreen(path, confidence=confidence, region=region)
                except Exception:
                    pass
    ...
    def _execute_logic_if(self, action: Dict[str, Any], current_index: int, run_actions: Optional[list] = None) -> Optional[int]:
        ...
        if condition == "image_found":
            ...
            if rgb:
                ...
                if 0 <= ty < ss.shape[0] and 0 <= tx < ss.shape[1]:
                    ...
                    rgb_arr = np.array(rgb, dtype=np.int16)
                    met = bool(np.all(np.abs(pixel[:3] - rgb_arr) <= tol))
        ...
        if should_jump:
            return self._find_label_index(str(target_label), run_actions)
        return None

    def _execute_logic_jump(self, action: Dict[str, Any], current_index: int, run_actions: Optional[list] = None) -> Optional[int]:
        target_label = action.get("target_label")
        self.log_message(f"[JUMP] กระโดดไปที่: {target_label}")
        return self._find_label_index(str(target_label), run_actions)

    def _find_label_index(self, label_name: str, run_actions: Optional[list] = None) -> Optional[int]:
        if not label_name:
            return None
        ...
        with self.variable_lock:
            ...
            try:
                ...
                if op == "add":
                    ...
                    current = current / change if change != 0 else 0

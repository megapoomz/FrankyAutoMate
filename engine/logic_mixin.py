...
class LogicMixin:
    ...
    def setup_logic_tab(self):
        ...
        ctk.CTkLabel(t, text="สร้างจุดอ้างอิง (Label)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="w", padx=22)
        ...
        f_l_row.pack(fill="x", padx=15, pady=15)
        self.entry_label_name = ctk.CTkEntry(f_l_row, placeholder_text="ตั้งชื่อจุด (เช่น: start_loop)", height=35, font=("Tahoma", 12))
        ...
        btn_label = ctk.CTkButton(f_l_row, text="วางจุดนี้", command=self.add_label_action, width=100, height=35, fg_color="#334155", font=("Tahoma", 11, "bold"))
        btn_label.pack(side="right")
        ToolTip(btn_label, "สร้างจุดอ้างอิง ณ ตำแหน่งนี้ของสคริปต์")

        # --- Section 2: If/Else Condition ---
        ctk.CTkLabel(t, text="ถ้า...ให้ไป... (Condition)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(20, 5), anchor="w", padx=22)
        ...
        f_if_row1.pack(fill="x", padx=15, pady=(15, 5))
        
        ctk.CTkLabel(f_if_row1, text="1. ถ้า (If):", font=("Tahoma", 12)).pack(side="left", padx=(0, 10))
        ...
        ctk.CTkLabel(f_if_row1, text="เป็นจริง", font=("Tahoma", 12, "bold"), text_color=COLOR_SUCCESS).pack(side="left", padx=10)
        ...
        self.lbl_logic_source_status = ctk.CTkLabel(self.f_logic_source, text="-", text_color="#94a3b8", font=("Tahoma", 11))
        ...
        ctk.CTkButton(t, text="เพิ่มเงื่อนไข (Add Condition)", command=self.add_if_action, 
                       fg_color="#9333ea", hover_color="#7e22ce", height=45, font=("Tahoma", 13, "bold")).pack(pady=10, padx=20, fill="x")

        # --- Section 3: Force Jump ---
        ctk.CTkLabel(t, text="สั่งกระโดด/วนลูป (Jump/Loop)", font=("Tahoma", 12, "bold"), text_color=COLOR_MUTED).pack(pady=(15, 5), anchor="w", padx=22)
        ...
        f_j_row.pack(fill="x", padx=15, pady=15)
        ctk.CTkLabel(f_j_row, text="กระโดดไปที่:", font=("Tahoma", 12)).pack(side="left", padx=(0, 10))
        ...
        ctk.CTkButton(f_j_row, text="เพิ่มคำสั่ง", command=self.add_jump_action, width=100, height=35, fg_color="#334155", font=("Tahoma", 11, "bold")).pack(side="right")
    ...
    def add_if_action(self):
        ...
        # --- 3. Block (IF/ELSE) Mode ---
            ...
            # Jump to End (Skip Else) - Placeholder for True Block end

...
class LogicMixin:
    ...
    def setup_logic_tab(self):
        ...
        btn_label = ctk.CTkButton(
            f_l_row, text="วางจุดนี้", command=self.add_label_action, width=100, height=35, fg_color="#334155", font=("Inter", 11, "bold")
        )
        ...
        self.slider_logic_conf = ctk.CTkSlider(
            self.f_logic_conf, from_=0.5, to=1.0, variable=self.var_logic_conf, command=update_logic_conf_lbl, height=16, width=150
        )
        ...
        self.opt_logic_struct = ctk.CTkOptionMenu(
            f_if_row2,
            variable=self.var_logic_struct,
            values=["Jump Only", "Block (IF)", "Block (IF/ELSE)"],
            command=self.update_logic_struct_ui,
            width=140,
        )
        ...
        ctk.CTkButton(
            t,
            text="เพิ่มเงื่อนไข (Add Condition)",
            command=self.add_if_action,
            fg_color="#9333ea",
            hover_color="#7e22ce",
            height=40,
            font=("Inter", 13, "bold"),
        ...
        ctk.CTkLabel(t, text="สั่งกระโดด/วนลูป (Jump/Loop)", font=("Inter", 12, "bold"), text_color=COLOR_MUTED).pack(
        ...
        ctk.CTkButton(
            f_j_row, text="เพิ่มคำสั่ง", command=self.add_jump_action, width=100, height=35, fg_color="#334155", font=("Inter", 11, "bold")
        ).pack(side="right")
    ...
    def update_logic_struct_ui(self, event=None):
        if not hasattr(self, "f_target_label"):
            return
        mode = self.var_logic_struct.get()
        if mode == "Jump Only":

...
class ActionMixin:
    ...
    def move_action_up(self) -> None:
        ...
        elif idx == 0: self.lbl_status.configure(text="⚠️ อยู่บนสุดแล้วครับ", text_color="#f1c40f")
        else: self.lbl_status.configure(text="⚠️ กรุณาคลิกเลือกขั้นตอนที่จะขยับก่อน", text_color="#f1c40f")
    ...
    def move_action_down(self) -> None:
        ...
        elif idx == len(self.actions) - 1: self.lbl_status.configure(text="⚠️ อยู่ล่างสุดแล้วครับ", text_color="#f1c40f")
        elif idx == -1: self.lbl_status.configure(text="⚠️ กรุณาคลิกเลือกขั้นตอนที่จะขยับก่อน", text_color="#f1c40f")
    ...
    def remove_selected_action(self) -> None:
        ...
        else: self.lbl_status.configure(text="⚠️ กรุณาคลิกเลือกขั้นตอนที่จะลบก่อน", text_color="#f1c40f")
    ...
    def add_click_action(self):
        ...
        self.add_action_item({
            ...
            "relative": is_rel, "mode": mode, "stop_after": stop_after
        })
    ...
    def add_type_action(self):
        ...
        if mode == "text": 
            self.add_action_item({
                "type": "text", 
                "content": txt,
                "mode": self.var_type_mode.get(),
                "stop_after": self.var_type_stop.get()
            })
        else: 
            self.add_action_item({
                "type": "hotkey", 
    ...
    def add_image_action(self):
        ...
        except (ValueError, TypeError): ox, oy = 0, 0
        self.add_action_item({
            "type": "image_search", "path": self.current_img_path,
            "region": self.current_region, "off_x": ox, "off_y": oy,
    ...
    def add_wait_action(self):
        ...
        except (ValueError, TypeError): s = 1.0
    ...
    def add_color_action(self):
        ...
        try: tol = int(self.entry_tol.get())
        except (ValueError, TypeError): tol = 10
            
        self.add_action_item({
            "type": "color_search", "x": abs_x, "y": abs_y, "rgb": rgb,
            "region": self.current_region, "tolerance": tol,
            "mode": self.var_color_mode.get(), "do_click": self.var_color_click.get(),
    ...
    def add_multi_color_action(self):
        ...
        if not points:
            ...
            return
        self.add_action_item({
            "type": "multi_color_check", "points": points.copy(),
            "logic": self.var_multi_color_logic.get(), "mode": "once",
